...
export function getCoordinatesByColor(
  ...
  const { saturation, brightness } = color.hsb;

  const X = width / 100;
  const Y = height / 100;

  const x = saturation * X;
  const y = (100 - brightness) * Y;
...
export function getColorByCoordinates(
  ...
  const hsb: ColorObject["hsb"] = { hue, saturation, brightness };
...
export function changeHue(color: ColorObject, newHue: number): ColorObject {
  const hsb: ColorObject["hsb"] = { ...color.hsb, hue: newHue };
